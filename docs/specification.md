...
- **Implementation note (v0.4.x, v0.5.x):** the reference implementation *hard-rejects* a relation reference (e.g. `revises_event_id`) that points outside the current session, at `append_event` time — stricter than the SHOULD above. Relaxing this to full spec-compliant dangling-tolerance is deferred to v1.1; producers should not rely on cross-session references until then.
